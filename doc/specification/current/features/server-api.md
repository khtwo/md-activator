...
- The server also accepts `--host` (default `127.0.0.1`), `-p`/`--port` (default `8000`), `--reload`/`--no-reload` (Uvicorn auto-reload, defaulting to off when the app is started directly), and `--no-use-colors` (disable Uvicorn color output).
