...
class MarkdownRenderer:
    ...
    def render(self, path: str | None, base: str | None = None) -> RenderResult:
        ...
        return RenderResult(
            ...
            html=html,
            links=links,
            files=self._list_files(md_path.parent),
            file_options=self._list_file_options(md_path.parent),
        )
    ...
    def _render_folder(self, folder: Path) -> RenderResult:
        ...
        return RenderResult(
            ...
            file_options=self._list_file_options(folder),
        )
    ...
    def _list_file_options(self, folder: Path) -> list[dict[str, str | bool | int]]:
        ...
        options.extend(self._list_file_tree_options(folder=folder, depth=0))
        return options

    def _list_file_tree_options(self, folder: Path, depth: int) -> list[dict[str, str | bool | int]]:
        options: list[dict[str, str | bool | int]] = []
        ...
        for path in sorted(folder.iterdir(), key=lambda entry: entry.name.lower()):
            if path.is_file() and path.suffix.lower() == ".md":
            ...
            elif path.is_dir() and self._folder_contains_markdown(path):
                relative_path = self._to_relative(path)
                options.append(
                    {
                        "label": path.name,
                ...
                if depth < max_depth:
                    options.extend(self._list_file_tree_options(folder=path, depth=depth + 1))

        return options

    def _folder_contains_markdown(self, folder: Path) -> bool:
        return any(path.is_file() and path.suffix.lower() == ".md" for path in folder.rglob("*"))
